...
def main() -> None:
    ...
    source_text = extract_text(input_path)
    if not source_text.strip():
        raise ValueError("Input text is empty")

    config = AppConfig.from_env(legal_workspace=input_path.parent, model=args.model)
    formatted_text = reformat_law_text(
...
def _build_user_prompt(source_text: str) -> str:
    return (
        "请将以下法律文档根据提供的例子重排为如下结构：\n"
        "第x章：章名\n"
        "第x节：节名\n"
        "第x条: 条文\n"
        "要求：\n"
        "1) 保留原文内容，不新增或删除事实\n"
        "2) 每条以“第x条:”开头，条文正文紧随其后或在下一行\n"
        "3) 章节与节标题必须使用“第x章：”“第x节：”格式\n"
        "4) 若原文无章/节划分，使用“第1章：未分章”“第1节：未分节”并收纳全部条文\n"
        "5) 输出只包含重排后的正文\n"
        "原文：\n"
        f"{source_text}\n"
    )
